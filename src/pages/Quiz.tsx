...
import { BrainCircuit, CheckCircle2, XCircle, Trophy, ChevronRight, RotateCcw } from 'lucide-react';
  ...
  if (!level) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        <BrainCircuit size={64} className="text-brand-600 dark:text-brand mb-6" />
        <h1 className="text-4xl font-black mb-4">Ready to test your knowledge?</h1>
        <p className="text-slate-500 mb-10 max-w-md">Choose a difficulty level to begin the quiz. Master the basics before moving to high-level system concepts.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-3xl">
          {(['Basic', 'Intermediate', 'High'] as const).map(l => (
            <button
              ...
              className="p-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl hover:border-brand hover:scale-105 transition-all text-left"
            >
              <div className="font-black text-sm mb-2 uppercase tracking-widest">{l}</div>
              <h3 className="text-2xl font-bold mb-2">{l} Level</h3>
  ...
  if (showResult) {
    return (
      <div className="max-w-xl mx-auto flex flex-col items-center justify-center py-20 text-center">
        <div className="w-24 h-24 bg-brand/20 dark:bg-brand/10 rounded-full flex items-center justify-center text-brand-700 dark:text-brand mb-8">
          <Trophy size={48} />
        </div>
        <h2 className="text-4xl font-black mb-2">Level Complete!</h2>
        <div className="text-6xl font-black my-8">
          {score} / {questions.length}
        </div>
        <p className="text-slate-500 mb-10">
          {score === questions.length ? "Incredible! You're a storage master." : "Great effort! Review the concepts to aim for a perfect score."}
        </p>
        <div className="flex gap-4">
          <button 
            onClick={() => handleLevelSelect(level)} 
            className="px-8 py-3 bg-brand-600 dark:bg-brand text-white rounded-xl font-bold flex items-center gap-2 hover:brightness-110 transition-all"
          >
            <RotateCcw size={20} /> Try Again
          </button>
          <button 
            onClick={() => setLevel(null)} 
            className="px-8 py-3 bg-slate-200 dark:bg-slate-800 rounded-xl font-bold"
          >
            Change level
          </button>
        </div>
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="bg-brand-600 dark:bg-brand text-white px-3 py-1 rounded-lg text-xs font-bold uppercase">
            {level}
          </div>
        ...
        <div className="h-2 flex-1 mx-8 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
           <motion.div 
            className="h-full bg-brand-600 dark:bg-brand"
            initial={{ width: 0 }}
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-xl">
        <h2 className="text-2xl md:text-3xl font-bold mb-10">{currentQuestion.text}</h2>
        ...
        <div className="space-y-4">
          {currentQuestion.options.map((option, idx) => (
            <button
              ...
              className={cn(
                "w-full p-6 rounded-2xl border-2 text-left transition-all flex items-center justify-between text-lg",
                isAnswered 
                  ...
                  : "border-slate-100 dark:border-slate-800 hover:border-brand hover:bg-brand/10 dark:hover:bg-brand/5"
              )}
            >
          ...
          {isAnswered && (
            ...
            >
              <h4 className="font-bold flex items-center gap-2 mb-2">
                <BrainCircuit size={18} /> Deep Insight
              </h4>
              <p className="text-slate-500 mb-8">{currentQuestion.explanation}</p>
              <button
                onClick={handleNext}
                className="w-full sm:w-auto px-10 py-4 bg-brand-600 dark:bg-brand text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:scale-105 transition-transform"
              >
                {currentIndex < questions.length - 1 ? "Next Question" : "View Results"} <ChevronRight size={20} />
              </button>
            </motion.div>
          )}
        </AnimatePresence>
